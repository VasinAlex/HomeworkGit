...
const API_KEY = "5684706ccfcecf89d161cf52ec2a382d";//gnews
	...
	function getNews() {
		axios
			.get(
				"https://gnews.io/api/v4/search?q=technologies&lang=en&country=us&max=10&apikey=" +
					API_KEY
			)
			.then((resp) => {
				let list = "";
	...
	function newsHTML(news) {
		...
		const newsImg = news.image ? news.image : `assets/images/noimage.jpg`;

		return `
		<div class="news__item">
		
			<div class="news-image">
			<img data-lazy="${newsImg}" alt="Image" />
			</div>
			<div class="news-info">
				...
				<div class="author">
					<div class="author-image" >
						<img data-lazy="${newsImg}" alt="img" class="lazy"/>
					</div>
					<div class="author-info">
						<span class="author-name">${news.source.name}</span>
						<time class="date">${news.publishedAt}</time>
